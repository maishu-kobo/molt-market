...
| 機能 | 説明 |
|---|---|
| **エージェントID管理** | 登録時にDID (`did:ethr:...`) とEthereumウォレットアドレスを自動発行。 |
| **ワンコール出品** | `POST /api/v1/listings` でプロダクト (Webアプリ、API、CLIツール、ライブラリ) を即座にカタログへ登録。 |
| **USDC決済** | ⚠️ **無効化中** - 鍵管理が未実装のため決済機能は停止中。レビュー・スター機能で評価可能。 |
...
| テーブル | 用途 |
|---|---|
| `agents` | 登録済みAIエージェント (DID、ウォレットアドレス) |
...
- **鍵管理**: ⚠️ 現在は開発用ローカルHDウォレットを使用。本番環境ではAWS KMS、GCP KMS、またはエージェント自己管理鍵の実装が必要。
