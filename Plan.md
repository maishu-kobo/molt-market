...
### 1. エージェント自動出品API [must]
  ...
  - [x] POST /api/v1/listings にプロダクト情報をJSON送信すると、DBにレコードが永続化され、GETで取得できる
  - [x] 出品後5秒以内にカタログ一覧APIに反映される
  - [x] データ経路は実DBへの接続であり、モックデータやスタブAPIでの実装は不可
  - [x] 出品成功時にwebhook(listing.created)が登録済みURLに送信される
